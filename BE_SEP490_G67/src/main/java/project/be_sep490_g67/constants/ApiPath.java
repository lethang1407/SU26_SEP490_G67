package project.be_sep490_g67.constants;

public class ApiPath {
    public static final String BASE_URL_V1 = "/api";
    public static final String USER = BASE_URL_V1 + "/users";
    public static final String AUTH = BASE_URL_V1 + "/auth";
    public static final String STORE = BASE_URL_V1 + "/store";
    public static final String STAFF = BASE_URL_V1 + "/staff";
    public static final String SUPPLIER = BASE_URL_V1 + "/suppliers";
    public static final String CATEGORY = BASE_URL_V1 + "/category";
    public static final String PRODUCTS = BASE_URL_V1 + "/products";
    public static final String CUSTOMERS = BASE_URL_V1 + "/customers";
    public static final String SALES_ORDERS = BASE_URL_V1 + "/sales-orders";
    public static final String IMPORT_ORDERS = BASE_URL_V1 + "/import-orders";
    public static final String STORAGE_LOCATIONS = BASE_URL_V1 + "/storage-locations";
    public static final String INVENTORY_ATTENTION = BASE_URL_V1 + "/inventory-attention";
    public static final String STORAGE_ZONES = BASE_URL_V1 + "/storage-zones";
    public static final String INVENTORY_CHECKS = BASE_URL_V1 + "/inventory-checks";
    public static final String STOCK_BATCHES = BASE_URL_V1 + "/stock-batches";
    public static final String IMPORT_RETURNS = BASE_URL_V1 + "/import-returns";
    public static final String DEBT_PAYMENTS_CUSTOMER = BASE_URL_V1 + "/debt-payments";
    public static final String IMPORT_HISTORY = BASE_URL_V1 + "/import-history";
    public static final String SALES_HISTORY = BASE_URL_V1 + "/sales-history";
    public static final String NOTIFICATIONS = BASE_URL_V1 + "/notifications";
    public static final String PAYMENT_METHODS = BASE_URL_V1 + "/payment";
    public static final String CHECKOUT = BASE_URL_V1 + "/checkout";
}
