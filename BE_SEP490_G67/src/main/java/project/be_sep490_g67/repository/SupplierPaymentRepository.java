...
public interface SupplierPaymentRepository extends JpaRepository<SupplierPayment, Integer> {
    ...
    /**
     * Số thứ tự lớn nhất trong ngày cho mã TTNddMMyy-xx (một dấu '-').
     * dayPrefix ví dụ: TTN210826. Bỏ qua mã cũ TTN000001.
     */
    @Query(value = """
            SELECT MAX(CAST(SUBSTRING(payment_code, LOCATE('-', payment_code) + 1) AS UNSIGNED))
            FROM supplier_payments
            WHERE payment_code LIKE CONCAT(:dayPrefix, '-%')
              AND payment_code NOT LIKE CONCAT(:dayPrefix, '-%-%')
            """, nativeQuery = true)
    Integer findMaxPaymentSequenceByDayPrefix(@Param("dayPrefix") String dayPrefix);
