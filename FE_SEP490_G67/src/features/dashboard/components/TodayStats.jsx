...
const buildRefundBadge = (salesSummary) => {
  const hasNetRevenue =
    salesSummary.netRevenue !== null && salesSummary.netRevenue !== undefined;
...
export default function TodayStats() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const result = await importHistoryApi.getSummary({
          from: today,
          to: today,
        });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const result = await dashboardApi.getReconciliationSummary({
          date: today,
        });
  ...
  const metrics = baseMetrics.map((metric) => {
    ...
    if (metric.label === "Tiền thực thu") {
      ...
      const deductions = [];
      if (cashRefunded > 0)
        deductions.push(`Hoàn khách: ${formatCompactCurrency(cashRefunded)}`);
      if (exchangeCredit > 0)
        deductions.push(
          `Cấn hàng đổi: ${formatCompactCurrency(exchangeCredit)}`,
        );
      return {
        ...metric,
        value: formatCompactCurrency(
          cash + bank - cashRefunded - exchangeCredit,
        ),
        note: (
          <>
            <div>Tiền mặt: {formatCompactCurrency(cash)}</div>
            <div>Chuyển khoản: {formatCompactCurrency(bank)}</div>
          </>
        ),
    ...
    if (metric.label === "Công nợ mới hôm nay") {
      return {
        ...metric,
        value: formatCompactCurrency(
          debtOverview?.totalDebtAmountIncurredToday,
        ),
        note: debtOverview
          ? `${debtOverview.totalDebtSalesCount ?? 0} đơn hàng ghi nợ`
          : null,
      };
    }
      ...
      <section className="metric-grid" aria-label="Chỉ số hôm nay">
        {metrics.map((metric) => (
          <article key={metric.label} className="metric-card">
            ...
            <div
              className={`metric-card_value metric-card_value--${metric.tone}`}
            >
              {metric.value}
            </div>
            {metric.badge && (
              <div
                className={`metric-card_badge metric-card_badge--${metric.badge.tone}`}
              >
                {metric.badge.text}
              </div>
            )}
            {metric.note && (
              <div className="metric-card_note">{metric.note}</div>
            )}
            {metric.warningBox && (
              <div className="metric-warning-box">
                <div className="metric-warning-box_title">
                  <AlertTriangle
                    size={12}
                    className="metric-warning-box_icon"
                  />
                  <span>{metric.warningBox.title}</span>
                </div>
                <div className="metric-warning-box_subtitle">
                  {metric.warningBox.subtitle}
                </div>
              </div>
            )}
          </article>
